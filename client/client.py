...
def check_weather_auto(latitude,longitude):
    ...
    with tracer.start_as_current_span(
        ...
        headers = {}
        inject(headers)
        requests.get(f"http://pyserver_auto:5000/checkweather?latitude={latitude}&longitude={longitude}",headers=headers)
...
def check_weather_manual(latitude,longitude):
    ...
    with tracer.start_as_current_span(
        ...
        headers = {}
        inject(headers)
        requests.get(f"http://pyserver_manual:5001/checkweather?latitude={latitude}&longitude={longitude}",headers=headers)
